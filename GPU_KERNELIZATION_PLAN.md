...
| Methane (CH4) | 1 | ~80 | **6 of ~7 process fns whole-fn on Metal; ch4! orchestrator (loop-kernelization) pending** | Foundation: `CH4Data` reparam'd `{FT,V,M,Vb}` + `@adapt_structure` (74 fields). DONE (Metal-validated): `get_jwt!`/`ch4_annualupdate!`/`ch4_totcolch4!` (diag, reductions), `ch4_oxid!`, `ch4_prod!` (per-patch rr_vr `_scatter_add!`), `ch4_aere!` (patch→col scatter), and **`ch4_tran!`** — the 2-species CH4/O2 reaction-diffusion vertical transport, the hardest fn in the module: ONE per-column kernel (competition + ebul/aere reductions + 2-species source/epsilon + jwt-branchy Patankar tridiag assembly + IN-THREAD Thomas solve ×2, inlined to match `tridiagonal_solve!`), `_Ch4TS`/`_Ch4TF`/`_Ch4Scr`/`_Ch4TP` device-view bundles. Pre-existing `meth_khcc!`/`meth_ebul!` kernels Metal-hardened (literals + scalar args). Done via a mix: foundation+`ch4_tran!`+orchestrator hands-on, the 4 mid-tier fns via a parallel workflow over disjoint methane.jl line-regions (cherry-picked). Gates: full suite 15681/15681 + 6 `gpu_validate_ch4*_e2e.jl` Metal harnesses at parity. **REMAINING: `ch4!` orchestrator** — kernelize its ~14 per-column/per-(c,j) data loops + the grnd_ch4_cond patch→col and the column→gridcell flux scatters (`_scatter_add!`), keep the sat/lake dispatch (calls the 6 device kernels) + conservation `@warn` checks host-side (materialize once). Orchestrator kernels drafted; the non-default lake path stays host. |
