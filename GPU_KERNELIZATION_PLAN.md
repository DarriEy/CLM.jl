...
| Decomposition & soil cycling | 8 | ~200 | **4 of 8 files whole-fn Metal-validated; 3 CPU-kernelized w/ Metal-hardening pending; 1 deferred (tridiag)** | Parallel workflow batch (7 disjoint-file agents). **DONE (whole-fn Metal at 0.0/round-off):** `decomp.jl` (`soil_biogeochem_decomp!` application), `decomp_competition.jl` (`soil_bgc_competition!`), `decomp_vertical_profile.jl` (`soil_bgc_vertical_profile!` — the TRUE patch→column scatter via `_scatter_add!`), `decomp_precision_control.jl` (`soil_bgc_precision_control!`). **PENDING Metal-hardening (CPU byte-identical/suite-green, host loops kernelized, but the PRE-EXISTING `_decompb_*`/`_decpot_*` kernels carry raw Float64 literals + unconverted Float64 scalar args → invalid Metal IR; the agents kernelized the host loops but didn't harden the old kernels):** `decomp_bgc.jl` (`decomp_rate_constants_bgc!`), `decomp_potential.jl` (`soil_bgc_potential!`). Both need: eltype-convert every literal in the preserved kernels + convert scalar args at launch. Already-fixed device-only bugs (banked): host scratch via `similar(state)` not `zeros()`; move `floating_cn_ratio`/`initial_cn_ratio`/cascade-pool vectors to the state backend (Bool/Int eltype preserved); `DecompBGCState`'s new matrix fields reparam'd `{FT,M<:AbstractMatrix{FT}}` + `{FT}` ctor for `adapt`. **`decomp_mimics.jl` (`decomp_rates_mimics!`): non-default MIMICS method; CPU byte-identical but harness blocked** — `init_decompcascade_mimics!` allocates texture arrays (fphys/desorp/p_scalar) at Float64 regardless of build precision, so a Float32 build mixes precisions in `_DMRArr` (reparam'd `DecompMIMICSState{M}` + `@adapt_structure` done; needs the texture arrays built at working precision). **`litter_vert_transp.jl` (tridiag) NOT attempted — layer-coupled implicit solve, deferred to a hands-on pass (soil-water `tridiagonal_multi!` template).** |
