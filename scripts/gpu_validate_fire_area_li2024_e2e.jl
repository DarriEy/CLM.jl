# ==========================================================================
# gpu_validate_fire_area_li2024_e2e.jl — end-to-end Metal parity for the WHOLE
# cnfire_area_li2024! burned-area driver (revised fire-occurrence formulation:
# prec30 running mean, wtgcell lfwt, crop-live-gated ag fire, tropical-tree
...
# at Float64, runs cnfire_area_li2024! on the CPU, adapts every kernel-touched
# struct to Metal/Float32, runs the SAME call on the device, and compares the
# mutated column burned-area diagnostics.
#
#   julia --project=scripts scripts/gpu_validate_fire_area_li2024_e2e.jl
# ==========================================================================
...
function build(; np=4, nc=2, ng=1, nlevgrnd=3, nlevdecomp=1, ndecomp_pools=4, n_litr=3)
    ...
    return (; pftcon, pftcon_li2014, cnfire_const, cnfire_params, fire_data, fire_li2014,
        ...
        prec30_patch=fill(2.5e-5,np), rh30_patch=fill(60.0,np),
        fsat_col=fill(0.1,nc), wf2_col=fill(0.25,nc), croplive_patch=falses(np),
...
run_area!(d) = CLM.cnfire_area_li2024!(
    ...
    fsat_col=d.fsat_col, wf2_col=d.wf2_col, croplive_patch=d.croplive_patch,
...
function main(backend)
    println("=" ^ 72)
    println("END-TO-END Metal parity for cnfire_area_li2024! (Li2024 burned area)")
    println("=" ^ 72)
    if backend === nothing
    ...
    D = (; fire_li2014=mf(B.fire_li2014), pftcon_li2014=mf(B.pftcon_li2014),
        ...
        prec30_patch=mf(B.prec30_patch), rh30_patch=mf(B.rh30_patch),
        fsat_col=mf(B.fsat_col), wf2_col=mf(B.wf2_col), croplive_patch=mb(B.croplive_patch),
    ...
    println()
    println(nfail == 0 ? "  WHOLE cnfire_area_li2024! MATCHES CPU ON $name ($FT)" :
