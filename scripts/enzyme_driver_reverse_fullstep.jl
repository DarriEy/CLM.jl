# =============================================================================
# FULL clm_drv! WHOLE-STEP reverse — canopy + the entire HydrologyNoDrainage block,
# 15 phases, through the PRODUCTION assembler CLM.driver_rev_phases:
#   canopy (energy balance) → soil_temperature! → [10-phase surface-hydrology block]
#   → soil_water! → water_table! → hydrology_no_drainage!
# Perturb t_grnd, seed h2osoi_vol, FD-validate dL/d(t_grnd) through the WHOLE timestep.
# Uses the production canopy_rev_aux/driver_rev_bundle/driver_rev_phases +
# compositional_reverse! engine (no script-local wrappers). The canopy block (12 Newton
# iters) makes this the slowest compile.  julia +1.10 --project=/tmp/clm_jl10_<id> this.jl
# =============================================================================
...
println("\n", "="^70)
@printf("FULL clm_drv! WHOLE-STEP REVERSE (%d phases, canopy+hydrology): rel=%.3e bracketed=%s  %s\n",
