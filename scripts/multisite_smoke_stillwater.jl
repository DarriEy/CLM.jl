...
# site (Stillwater, Oklahoma: semi-arid C4 grassland / cropland, hot continental
# climate, ~97 kPa) from a cold start with a synthetic but physically-reasonable
# hot-dry SUMMER continental forcing snapshot, and checks the driver runs
# without NaN/blowup.
#
...
# 3 PFTs) by exercising a radically different subgrid + climate. Companion to
# scripts/multisite_smoke.jl (tropical Aripuanã Amazon).
#
# Usage: julia +1.12 --project=. scripts/multisite_smoke_stillwater.jl
# =============================================================================
...
function set_semiarid_forcing!(inst, ng, nc)
    a = inst.atm2lnd
    # Hot, dry, sunny mid-summer continental afternoon over the Southern Plains.
    T = 305.0; pbot = 97000.0; q = 0.010
    ...
    for g in 1:ng
        ...
        a.forc_lwrad_not_downscaled_grc[g]= 380.0    # lower downwelling LW (dry sky)
        a.forc_rain_not_downscaled_grc[g] = 0.0      # dry: no rain
        a.forc_snow_not_downscaled_grc[g] = 0.0      # summer: no snow
        a.forc_u_grc[g] = 3.0; a.forc_v_grc[g] = 0.0
        a.forc_hgt_grc[g] = 30.0
        # CRITICAL: 30 m reference heights keep the M-O friction-velocity solve
        # from blowing up to NaN on a cold start.
        ...
        a.forc_topo_grc[g] = 270.0   # Stillwater ~270 m a.s.l.
        # solar: 2 numrad bands (vis, nir); strong clear-sky midday values
        for b in 1:size(a.forc_solad_not_downscaled_grc, 2)
            a.forc_solad_not_downscaled_grc[g, b] = b == 1 ? 800.0 : 400.0
            a.forc_solai_grc[g, b]                = b == 1 ? 120.0 : 80.0
        end
    end
    for c in 1:nc; inst.topo.topo_col[c] = 270.0; end
    return nothing
end
...
function main(; nsteps::Int = 6, use_cn::Bool = false)
    ...
    start_date = DateTime(2006, 7, 1, 12)   # midday, mid-summer
    ...
    @printf("Stillwater OK subgrid: ng=%d nc=%d np=%d  PFTs=%s\n", ng, nc, np, string(Int.(inst.patch.itype)))
    ...
    nan_fields = String[]
    for i in 1:nsteps
        set_semiarid_forcing!(inst, ng, nc)
    ...
    println(isempty(nan_fields) ? "\n✅ MULTI-SITE SMOKE PASS (use_cn=$use_cn): driver ran $nsteps steps at Stillwater OK, all finite" :
                                  "\n❌ NaN appeared at: $(nan_fields)")
    return (inst=inst, bounds=bounds, nbad=length(nan_fields))
end

# true iff site data present AND both biogeophys + full-BGC runs stayed finite.
function multisite_stillwater_ok(; nsteps::Int = 6)
    isfile(FS) || (@info "Stillwater smoke: surfdata absent, skipping" FS; return missing)
