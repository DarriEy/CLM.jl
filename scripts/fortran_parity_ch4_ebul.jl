# ==========================================================================
# METHANE (ch4) Fortran parity — CH4 EBULLITION (CH4_EBUL_*), backlog A1 residual.
#
# THE UNLOCK (docs/CH4_FIRE_PARITY.md §13)
...
#   CONC_CH4_SAT (~0.029 mol/m^3) gives vgc ~= 0.045, below threshold, so
#   CH4_EBUL_* is identically zero at EVERY on-disk site and the ebullition kernel's
#   firing regime was never diffed against Fortran.
#
#   THE REFERENCE (SYMFLUENCE_data/clm_bgc_spinup/bow_ref_ch4ebul): a short startup
#   continuation from the ch4tws warm restart at 2202-07-24-14400, with CONC_CH4_SAT/
#   UNSAT SCALED x5 in the finidat (finidat_ch4ebul_x5.nc) so the saturated column
#   crosses the bubble threshold and `ch4_ebullition` fires (Fortran
#   CH4_EBUL_TOTAL_SAT = 1.21e-6 at n0, decaying as transport dissipates the conc).
#   Both CLM.jl and CTSM compute ebullition from the IDENTICAL scaled before-step
#   concentration (read from the pdump), so the diff isolates the _meth_ebul_kernel!
#   physics — the synthetic-shared-input technique of the peatf (§12) and lnfm/hdm
#   (§2) references, applied to a state variable instead of a boundary parameter.
#
#   window : nstep 0..6 (2202-07-24 04:00 .. 10:00), boundary after_ch4.
#   n0 carries the pristine scaled conc (highest ebul signal); later steps decay.
#
# ---------------------------------------------------------------------------
# (Inherited from the TWS_inversion source harness: finundated>0 via satellite
# regression, warm CN. See fortran_parity_ch4_tws.jl for the finundation port.)
#
...
const DUMPDIR_CH4 = "/Users/darri.eythorsson/compHydro/SYMFLUENCE_data/clm_bgc_spinup/bow_ref_ch4ebul"
...
const N0    = 0
const NLAST = 6
const DATE0 = DateTime(2003, 7, 24, 4)            # DOY-matched to model 2202-07-24 04:00 (get_nstep=0)
...
# model 2202-07-24 04:00 (get_nstep=0) -> forcing 2002-07-24 04:00. Unlike the recycled
# long-run datm (-1 h, §5), this FRESH STARTUP aligns the datm record to the model hour
# EXACTLY (offset 0): verified GRND_CH4_COND bit-exact (J=F=1.03706e-2, rel 1.7e-11) at
# offset 0, so the forcing-sensitive aerenchyma/conductance path is aligned (§11 Bug 1).
const FORCE_OFFH   = parse(Int, get(ENV, "CH4_FORCE_OFFH", "0"))
const FORCE_DATE0  = DateTime(2002, 7, 24, 4) + Hour(FORCE_OFFH)
...
function main()
    ...
    steps  = collect(N0:min(N0 + nprobe - 1, NLAST))   # include n0 (pristine scaled conc, highest ebul)
    println("="^78)
    println("METHANE (ch4) FORTRAN PARITY — Bow EBULLITION (conc x5 -> vgc>0.0855), boundary after_ch4")
    println("  reference : ", DUMPDIR_CH4)
    println("  window    : nstep ", N0, "..", NLAST, "  (probing ", length(steps), " steps)")
    println("  finundated: ~0.045 (TWS_inversion) — CH4_EBUL_* NON-vacuous for the first time")
    println("="^78)
