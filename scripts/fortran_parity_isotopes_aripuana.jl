...
# TROPICAL variant — Aripuanã (Amazon, broadleaf-evergreen tropical). Cold-start
# CN+c13+c14 run (model year 2002, forcing year 2002; NO recycle offset) generated
# in /private/tmp/claude-501/ari_iso_run. Same teacher-forced single-step diff as
# the Bow harness, but at the canonical strong-δ13C-discrimination stress site.
const ISO_DUMPDIR = get(ENV, "ARI_ISO_DUMPDIR", "/private/tmp/claude-501/ari_iso_run")
const ARI_DATA    = "/Users/darri.eythorsson/projects/scratch/aripuana_data"
const ARI_SURF    = joinpath(ARI_DATA, "params/surfdata_clm.nc")
const ARI_PARAM   = joinpath(ARI_DATA, "params/clm5_params.nc")
const ISO_FORCING = joinpath(ARI_DATA, "forcing/clmforc.2002.nc")
...
function iso_parity_step(nstep)
    ...
    try
        inst, bounds = run_one_parity_step!(nstep; use_cn=true, dumpdir=ISO_DUMPDIR,
            ...
            fsurdat=ARI_SURF, paramfile=ARI_PARAM, natpft_size=15,
    ...
    # ---- NaN-origin probe (patch 2 = BET tropical): trace the dependency chain
    # bulk cpool/leafc → cpool_13 → leafc_13 to see which quantity first goes NaN.
    if get(ENV,"ISO_NANPROBE","")=="1"
        bcs  = inst.bgc_vegetation.cnveg_carbonstate_inst
        c13  = inst.bgc_vegetation.c13_cnveg_carbonstate_inst
        bcf  = inst.bgc_vegetation.cnveg_carbonflux_inst
        c13f = hasproperty(inst.bgc_vegetation,:c13_cnveg_carbonflux_inst) ? inst.bgc_vegetation.c13_cnveg_carbonflux_inst : nothing
        p = 2
        gp(o,f) = (hasproperty(o,f) && length(getfield(o,f))>=p) ? Float64(getfield(o,f)[p]) : NaN
        println("\n  [NANPROBE patch $p] BULK: cpool=", gp(bcs,:cpool_patch), " leafc=", gp(bcs,:leafc_patch),
                " leafc_storage=", gp(bcs,:leafc_storage_patch), " leafc_xfer=", gp(bcs,:leafc_xfer_patch))
        println("  [NANPROBE] C13 state: cpool_13=", gp(c13,:cpool_patch), " leafc_13=", gp(c13,:leafc_patch),
                " leafc_storage_13=", gp(c13,:leafc_storage_patch), " leafc_xfer_13=", gp(c13,:leafc_xfer_patch))
        for f in (:cpool_to_leafc_patch,:leafc_xfer_to_leafc_patch,:psnsun_to_cpool_patch,:cpool_to_leafc_storage_patch)
            print("  [NANPROBE] bulk flux ", f, "=", gp(bcf,f))
            c13f !== nothing && print("   c13 flux=", gp(c13f,f))
            println()
        end
    end
