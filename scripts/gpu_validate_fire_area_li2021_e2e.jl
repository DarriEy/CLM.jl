...
# at Float64, runs cnfire_area_li2021! on the CPU, adapts every kernel-touched
# struct to Metal/Float32, runs the SAME call on the device, and compares the
# mutated column burned-area diagnostics.
#
#   julia --project=scripts scripts/gpu_validate_fire_area_li2021_e2e.jl
# ==========================================================================
...
run_area!(d) = CLM.cnfire_area_li2021!(
...
function main(backend)
    ...
    println()
    println(nfail == 0 ? "  WHOLE cnfire_area_li2021! MATCHES CPU ON $name ($FT)" :
