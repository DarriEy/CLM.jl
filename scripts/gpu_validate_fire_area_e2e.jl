...
# burned-area fraction, and the main per-column fire-spread loop).
#
# Builds the fire dataset (clone of test_fire_li2014's make_fire_li2014_data) at
# Float64, runs cnfire_area_li2014! on the CPU, adapts every kernel-touched struct
# to Metal/Float32, runs the SAME call on the device, and compares the mutated
# column burned-area diagnostics.
#
#   julia --project=scripts scripts/gpu_validate_fire_area_e2e.jl
# ==========================================================================
...
function build(; np=4, nc=2, ng=1, nlevgrnd=3, nlevdecomp=1, ndecomp_pools=4, n_litr=3)
    ...
    for f in (:rootc_col,:leafc_col,:fuelc_col,:fuelc_crop_col); setfield!(cnveg_cs,f,zeros(nc)); end
    ...
    return (; pftcon, pftcon_li2014, cnfire_const, cnfire_params, fire_data, fire_li2014,
        ...
        fsat_col=fill(0.1,nc), wf_col=fill(0.3,nc), wf2_col=fill(0.25,nc),
...
run_area!(d) = CLM.cnfire_area_li2014!(
...
const OUT_FIELDS = (:farea_burned_col, :nfire_col, :fbac_col, :fbac1_col, :baf_crop_col,
                    :baf_peatf_col, :fsr_col, :fd_col, :lgdp_col, :lpop_col,
...
function main(backend)
    ...
    D = (; fire_li2014=mf(B.fire_li2014), pftcon_li2014=mf(B.pftcon_li2014),
        ...
        prec60_patch=mf(B.prec60_patch), prec10_patch=mf(B.prec10_patch),
        fsat_col=mf(B.fsat_col), wf_col=mf(B.wf_col), wf2_col=mf(B.wf2_col),
    ...
    println()
    println(nfail == 0 ? "  WHOLE cnfire_area_li2014! MATCHES CPU ON $name ($FT)" :
