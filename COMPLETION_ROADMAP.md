...
- **Tier F — FATES** (Functionally Assembled Terrestrial Ecosystem Simulator) — ~77k lines of Fortran / 68 files under `installs/clm/src/fates/` (main/biogeochem/biogeophys/parteh/radiation/fire). Greenfield → `src/fates/`. Dependency-ordered wave plan (each wave merges before the next; modules within a wave are mutually independent → parallel agents):
  ...
  - **F-Batch 14 — ✅ done (PR #76):** EDPatchDynamicsMod (3.9k — the patch disturbance engine; ALL 16 subroutines + 2 helpers: disturbance_rates, spawn/split/fuse/terminate patches, fire/mortality/landusechange_litter_fluxes mass-conserving, DistributeSeeds, set_patchno/countPatches/check_patch_area/patch_pft_size_profile/GetPseudoPatchAge; reuses Batch 12–13 cohort/mortality/logging/physiology helpers; plant-hydro init paths stubbed behind hlm_use_planthydro; 39t). suite 20,088→20,127. **F-Batch 15:** EDCanopyStructureMod → structure checkpoint. **F-Batch 16 (4):** EDMainMod (driver), FatesInventoryInit, FatesNormanRad, FatesPlantRespPhotosynth. **F-Batch 17 (2):** EDInitMod, FatesRadiationDrive → init checkpoint.
