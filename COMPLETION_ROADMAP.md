...
- **Tier F — FATES** (Functionally Assembled Terrestrial Ecosystem Simulator) — ~77k lines of Fortran / 68 files under `installs/clm/src/fates/` (main/biogeochem/biogeophys/parteh/radiation/fire). Greenfield → `src/fates/`. Dependency-ordered wave plan (each wave merges before the next; modules within a wave are mutually independent → parallel agents):
  ...
  - **F-Batch 11 (8, ~11.5k):** ChecksBalances, EDAccumulateFluxes, FatesLandUseChange, FatesPlantHydraulicsMod (6.4k), FatesSoilBGCFlux, FatesTwoStreamUtils, PRTParamsFATES, SFMainMod. **F-Batch 12 — ✅ done (PR #74):** EDBtranMod (water-stress btran + root-soil uptake; 20t, no stubs), EDCohortDynamicsMod (1.9k — demographic cohort engine: create/insert/sort/count/fuse/terminate linked list + EvaluateAndCorrectDBH/DamageRecovery/SendCohortToLitter; plant-hydro fuse/init helpers stubbed behind hlm_use_planthydro; 37t), EDLoggingMortalityMod (1.3k — logging/harvest mortality fractions + killed-biomass→CWD/litter+wood-product + harvest-debt; 46t). 103t; suite 19,683→19,999. **F-Batch 13 (3):** EDMortalityFunctions, EDPhysiologyMod (3.4k), FatesBstress → physiology checkpoint.
