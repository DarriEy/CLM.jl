...
|---|------|-----------------|----------------------------------|---------------------------|
| A1 | **Methane as a SOURCE** (transport / aerenchyma / ebullition / `finundated>0`) — the regime CH4 actually matters for | `methane.jl` | `use_lch4=.true.` at a site with `frac_h2osfc>0` so `finundated>0`; diff `CH4_EBUL_*`, `CH4_AERE_*`, `CH4_SURF_*`, `CONC_CH4/O2_*`, `TOTCOLCH4`. At Bow these are vacuous (`finundated≡0`). **BLOCKED 2026-07-17 (PR #234): `domain_Peatland_MerBleue_Canada` is ABSENT from local `SYMFLUENCE_data` — Phase 1 could not configure/run. Restore the domain to proceed.** While blocked, fixed CH4 lead (a) `grnd_ch4_cond` wiring (verified vs Fortran, gated) and isolated the dominant remaining bug: the **saturated-column collapse** (`CONC_CH4_SAT→0` after one Julia step). See docs/CH4_FIRE_PARITY.md §6/§7. | `domain_Peatland_MerBleue_Canada` (**MISSING**) |
