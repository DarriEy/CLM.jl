...
| `fortran_parity_isotopes.jl` | `carbon_isotopes.jl` / `c_iso_flux.jl` | c13/c14 veg pools + one-step increment + **δ13C** | **bgc_ref_iso n4700-4725** | Per-isotope `c_state_update{0,1,2,2h,2g,3}!` cascade now WIRED + isotope psn→cpool input repaired. Post-step pool parity **8.9e-6**, `cpool_13/14` **EXACT (0.0)**, **δ13C discrimination parity 9.4e-5 ‰** (the isotope-physics test, decoupled from bulk-C). One-step increment |rel| ~1.19 is the single-step bulk-C oracle floor on near-static veg pools (leafc net Δ ~1e-7). | **VALIDATED (isotope discrimination physics)** — increment on near-static pools sits at the inherited bulk-C summer residual floor |
| `fortran_parity_lake.jl` | `lake_*.jl` (con/fluxes/hydrology/temperature) | TG/TLAKE/LAKEICEFRAC/FSH/EFLX_LH/FIRE/FSA/... | **clm_lake_run h0 REGENERATED (2026-07-18)** via `scripts/validation/gen_lake_ref.sh` (cold-start SP, PCT_LAKE=100, 48 hourly steps) | rc=0: TLAKE/LAKEICE track ~1%; FSH/EFLX_LH ~10%, TSA ~7–11% (surface-flux residual); FSA blows up relative only at near-zero dawn/dusk | **VALIDATED (thermodynamics) / DIVERGES (surface turbulent flux)** — reference restored; residual matches the historical open finding |
