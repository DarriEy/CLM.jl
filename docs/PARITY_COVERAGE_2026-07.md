...
| `fortran_parity_isotopes.jl` | `carbon_isotopes.jl` / `c_iso_flux.jl` / `irrigation.jl` | (isotope path) + CN pools on irrigate=T | tier1_iso n1757873 | c13/c14 **Fortran branch CRASHES** at restart read (no c13/c14 vars); irrigate=T runs but inactive (no crop CFT); CN==baseline ~1.9e-4 | **NEEDS-FORTRAN-RUN** — requires a c13/c14-enabled spinup |
