...
#
# NOTE: This is a non-default fire method. Per the project convention it is
# written with straightforward mask-based host loops (no KA kernels); the
# default li2014 path is untouched and stays byte-identical.
# ==========================================================================
...
function cnfire_area_li2016!(
    ...
    for p in bounds_p
        ...
        if patch.itype[p] < nc3crop && cropf_col[c] < 1.0
            btran_col[c] += btran2[p] * patch.wtcol[p]
            wtlf[c]      += patch.wtcol[p]
        end
    end

    # --- main noncrop per-patch column-var accumulation ---
    for p in bounds_p
        mask_soilp[p] || continue
        c = patch.column[p]
        g = col.gridcell[c]
        if patch.itype[p] < nc3crop && cropf_col[c] < 1.0
            ...
            # NOTE: Li2016 weights deadcrootc by spinup_factor_deadwood (unlike Li2014).
            ...
            if hdmlf > 0.1
                if patch.itype[p] != noveg
                    if patch.itype[p] >= nbrdlf_evr_shrub  # shrub + grass
    ...
    # --- burned-area fraction in cropland ---
    for c in bounds_c
        mask_soilc[c] || continue
        baf_crop[c] = 0.0
    end
    ...
    for p in bounds_p
        ...
        if forc_t[c] >= TFRZ && patch.itype[p] > nc4_grass &&
           kmo == abm_lf[c] && burndate[p] >= 999 && patch.wtcol[p] > 0.0
            hdmlf = forc_hdm[g]
            # human density impact on ag. fire (Li2016 form)
            ...
            # NOTE(Li2016): ag-fire baf omits the fuel factor fb (only fb gates burndate)
    ...
    for c in bounds_c
        ...
        if cropf_col[c] < 1.0
            ...
            if spinup_state == 2
                # AD-spinup: re-scale CWD + dead-stem fuel by spinup factors
            ...
            if trotr1_col[c] + trotr2_col[c] <= 0.6
                ...
                if forc_rh[g] < rh_hgh && wtlf[c] > 0.0 && tsoi17[c] > TFRZ
                    fire_m = ((afuel * arh30 + (1.0 - afuel) * arh)^1.5) *
                             ((1.0 - max(0.0, min(1.0,
                                (btran_col[c] / wtlf[c] - bt_min) / (bt_max - bt_min))))^0.5)
                else
                    fire_m = 0.0
                end
                lh = pot_hmn_ign_counts_alpha * 6.8 * hdmlf^0.43 / 30.0 / 24.0
                fs = 1.0 - (0.01 + 0.98 * exp(-0.025 * hdmlf))
                # NOTE(Li2016): lightning normalization uses latitude in DEGREES
                # (cos(pi/180 * 3 * min(60, |latdeg|))), unlike Li2014's radians.
                ig = (lh + forc_lnfm[g] /
