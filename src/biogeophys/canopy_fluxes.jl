...
function canopy_fluxes_core!(
    ...
    # PHS consumes the carried smp_l/hk_l from the previous hydrology step, then fills
    # k_soil_root for the photosynthesis calcstress!/vegwp solve.
    if use_hydrstress
        ...
        # PHS photosynthesis consumes the carried soil hydraulic state from the
        # previous hydrology step. Preserve finite persisted/restart values for
        # Fortran parity; recompute only as a cold-start/fallback path.
    ...
    while itlef <= ctrl.itmax_canopy_fluxes && any(active)
        ...
        if CANOPY_PERITER_DEBUG[]
            ...
            open(CANOPY_PERITER_PATH[], itlef == 0 ? "w" : "a") do io
                if itlef == 0
                    println(io, "nstep itlef p t_veg dt_veg del del2 efe efsh ustar um obu wtl0 wtg0 wtga wtgaq qsatl qsatldT dc1 dc2 rssun rssha tlbef sabv air bir cir qg forc_q rb uaf ram1 qg_snow qg_soil qg_h2osfc frac_sno_eff frac_h2osfc frac_sno snow_depth int_snow h2osno_total qflx_snow_grnd qflx_snow_drain")
                end
                for fi in 1:fn
                    p = filterp[fi]
                    active[p] || continue
                    ...
                    row = (CANOPY_PERITER_NSTEP[], itlef + 1, p,
                        ...
                        waterfluxbulk.wf.qflx_snow_grnd_col[c],
                        waterfluxbulk.wf.qflx_snow_drain_col[c])
                    println(io, join(row, " "))
                end
            end
        end
