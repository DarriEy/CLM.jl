...
Base.@kwdef mutable struct SoilBiogeochemCarbonStateData{FT<:Real,
    ...
    decomp_cpools_vr_col              ::A3 = Array{Float64}(undef, 0, 0, 0)  # (gC/m3)
    decomp0_cpools_vr_col             ::A3 = Array{Float64}(undef, 0, 0, 0)  # (gC/m3) baseline (initial value of this year)
    decomp_cpools_vr_SASUsave_col     ::A3 = Array{Float64}(undef, 0, 0, 0)  # (gC/m3) SASU save
    decomp_soilc_vr_col               ::M = Matrix{Float64}(undef, 0, 0)    # (gC/m3) total soil C vr
    ctrunc_vr_col                     ::M = Matrix{Float64}(undef, 0, 0)    # (gC/m3) C truncation vr

    # --- Summary (diagnostic) state variables ---
    ctrunc_col                        ::V = Float64[]  # (gC/m2) C truncation
    totmicc_col                       ::V = Float64[]  # (gC/m2) total microbial C
    totlitc_col                       ::V = Float64[]  # (gC/m2) total litter C
    totlitc_1m_col                    ::V = Float64[]  # (gC/m2) total litter C to 1m
    totsomc_col                       ::V = Float64[]  # (gC/m2) total SOM C
    totsomc_1m_col                    ::V = Float64[]  # (gC/m2) total SOM C to 1m
    cwdc_col                          ::V = Float64[]  # (gC/m2) coarse woody debris C
    decomp_cpools_1m_col              ::M = Matrix{Float64}(undef, 0, 0)  # (gC/m2) decomp pools to 1m
    decomp_cpools_col                 ::M = Matrix{Float64}(undef, 0, 0)  # (gC/m2) decomp pools
    dyn_cbal_adjustments_col          ::V = Float64[]  # (gC/m2) dynamic column area adjustments
    ...
    totc_col                          ::V = Float64[]  # (gC/m2) total column C
    totecosysc_col                    ::V = Float64[]  # (gC/m2) total ecosystem C
    totc_grc                          ::V = Float64[]  # (gC/m2) total gridcell C

    # --- Matrix-CN fields ---
    matrix_cap_decomp_cpools_col      ::M = Matrix{Float64}(undef, 0, 0)    # (gC/m2)
    matrix_cap_decomp_cpools_vr_col   ::A3 = Array{Float64}(undef, 0, 0, 0)  # (gC/m3)
    in_acc                            ::M = Matrix{Float64}(undef, 0, 0)    # (gC/m3/yr)
    in_acc_2d                         ::A3 = Array{Float64}(undef, 0, 0, 0)  # (gC/m3/yr)
    tran_acc                          ::A3 = Array{Float64}(undef, 0, 0, 0)  # (gC/m3/yr)
    vert_up_tran_acc                  ::A3 = Array{Float64}(undef, 0, 0, 0)  # (gC/m3/yr)
    vert_down_tran_acc                ::A3 = Array{Float64}(undef, 0, 0, 0)  # (gC/m3/yr)
    exit_acc                          ::A3 = Array{Float64}(undef, 0, 0, 0)  # (gC/m3/yr)
    hori_tran_acc                     ::A3 = Array{Float64}(undef, 0, 0, 0)  # (gC/m3/yr)
end

SoilBiogeochemCarbonStateData{FT}(; kwargs...) where {FT<:Real} =
    SoilBiogeochemCarbonStateData{FT, Vector{FT}, Matrix{FT}, Array{FT,3}}(; kwargs...)
Adapt.@adapt_structure SoilBiogeochemCarbonStateData
