...
"""
Base.@kwdef mutable struct CNVegNitrogenStateData
    # --- Patch-level 2D (patch x nrepr) ---
    reproductiven_patch               ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)  # (gN/m2) reproductive (grain) N
    reproductiven_storage_patch       ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)  # (gN/m2) reproductive N storage
    reproductiven_xfer_patch          ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)  # (gN/m2) reproductive N transfer

    # --- Patch-level 1D core pools ---
    leafn_patch                       ::Vector{Float64} = Float64[]  # (gN/m2) leaf N
    leafn_storage_patch               ::Vector{Float64} = Float64[]  # (gN/m2) leaf N storage
    leafn_xfer_patch                  ::Vector{Float64} = Float64[]  # (gN/m2) leaf N transfer
    leafn_storage_xfer_acc_patch      ::Vector{Float64} = Float64[]  # (gN/m2) accumulated leaf N transfer
    storage_ndemand_patch             ::Vector{Float64} = Float64[]  # (gN/m2) N demand during offset period
    frootn_patch                      ::Vector{Float64} = Float64[]  # (gN/m2) fine root N
    frootn_storage_patch              ::Vector{Float64} = Float64[]  # (gN/m2) fine root N storage
    frootn_xfer_patch                 ::Vector{Float64} = Float64[]  # (gN/m2) fine root N transfer
    livestemn_patch                   ::Vector{Float64} = Float64[]  # (gN/m2) live stem N
    livestemn_storage_patch           ::Vector{Float64} = Float64[]  # (gN/m2) live stem N storage
    livestemn_xfer_patch              ::Vector{Float64} = Float64[]  # (gN/m2) live stem N transfer
    deadstemn_patch                   ::Vector{Float64} = Float64[]  # (gN/m2) dead stem N
    deadstemn_storage_patch           ::Vector{Float64} = Float64[]  # (gN/m2) dead stem N storage
    deadstemn_xfer_patch              ::Vector{Float64} = Float64[]  # (gN/m2) dead stem N transfer
    livecrootn_patch                  ::Vector{Float64} = Float64[]  # (gN/m2) live coarse root N
    livecrootn_storage_patch          ::Vector{Float64} = Float64[]  # (gN/m2) live coarse root N storage
    livecrootn_xfer_patch             ::Vector{Float64} = Float64[]  # (gN/m2) live coarse root N transfer
    deadcrootn_patch                  ::Vector{Float64} = Float64[]  # (gN/m2) dead coarse root N
    deadcrootn_storage_patch          ::Vector{Float64} = Float64[]  # (gN/m2) dead coarse root N storage
    deadcrootn_xfer_patch             ::Vector{Float64} = Float64[]  # (gN/m2) dead coarse root N transfer
    retransn_patch                    ::Vector{Float64} = Float64[]  # (gN/m2) plant pool of retranslocated N
    npool_patch                       ::Vector{Float64} = Float64[]  # (gN/m2) temporary plant N pool
    ntrunc_patch                      ::Vector{Float64} = Float64[]  # (gN/m2) patch-level sink for N truncation
    cropseedn_deficit_patch           ::Vector{Float64} = Float64[]  # (gN/m2) pool for seeding new crop growth (negative)

    # --- Matrix CN capacity fields (patch-level) ---
    matrix_cap_repron_patch             ::Vector{Float64} = Float64[]  # (gN/m2) capacity of grain N
    matrix_cap_repron_storage_patch     ::Vector{Float64} = Float64[]  # (gN/m2) capacity of grain N storage
    matrix_cap_repron_xfer_patch        ::Vector{Float64} = Float64[]  # (gN/m2) capacity of grain N transfer
    matrix_cap_leafn_patch              ::Vector{Float64} = Float64[]  # (gN/m2) capacity of leaf N
    matrix_cap_leafn_storage_patch      ::Vector{Float64} = Float64[]  # (gN/m2) capacity of leaf N storage
    matrix_cap_leafn_xfer_patch         ::Vector{Float64} = Float64[]  # (gN/m2) capacity of leaf N transfer
    matrix_cap_frootn_patch             ::Vector{Float64} = Float64[]  # (gN/m2) capacity of fine root N
    matrix_cap_frootn_storage_patch     ::Vector{Float64} = Float64[]  # (gN/m2) capacity of fine root N storage
    matrix_cap_frootn_xfer_patch        ::Vector{Float64} = Float64[]  # (gN/m2) capacity of fine root N transfer
    matrix_cap_livestemn_patch          ::Vector{Float64} = Float64[]  # (gN/m2) capacity of live stem N
    matrix_cap_livestemn_storage_patch  ::Vector{Float64} = Float64[]  # (gN/m2) capacity of live stem N storage
    matrix_cap_livestemn_xfer_patch     ::Vector{Float64} = Float64[]  # (gN/m2) capacity of live stem N transfer
    matrix_cap_deadstemn_patch          ::Vector{Float64} = Float64[]  # (gN/m2) capacity of dead stem N
    matrix_cap_deadstemn_storage_patch  ::Vector{Float64} = Float64[]  # (gN/m2) capacity of dead stem N storage
    matrix_cap_deadstemn_xfer_patch     ::Vector{Float64} = Float64[]  # (gN/m2) capacity of dead stem N transfer
    matrix_cap_livecrootn_patch         ::Vector{Float64} = Float64[]  # (gN/m2) capacity of live coarse root N
    matrix_cap_livecrootn_storage_patch ::Vector{Float64} = Float64[]  # (gN/m2) capacity of live coarse root N storage
    matrix_cap_livecrootn_xfer_patch    ::Vector{Float64} = Float64[]  # (gN/m2) capacity of live coarse root N transfer
    matrix_cap_deadcrootn_patch         ::Vector{Float64} = Float64[]  # (gN/m2) capacity of dead coarse root N
    matrix_cap_deadcrootn_storage_patch ::Vector{Float64} = Float64[]  # (gN/m2) capacity of dead coarse root N storage
    matrix_cap_deadcrootn_xfer_patch    ::Vector{Float64} = Float64[]  # (gN/m2) capacity of dead coarse root N transfer

    # --- Initial pool size of year for matrix (SASU) ---
    leafn0_patch                      ::Vector{Float64} = Float64[]  # (gN/m2) initial leaf N for SASU
    leafn0_storage_patch              ::Vector{Float64} = Float64[]  # (gN/m2) initial leaf N storage for SASU
    leafn0_xfer_patch                 ::Vector{Float64} = Float64[]  # (gN/m2) initial leaf N transfer for SASU
    frootn0_patch                     ::Vector{Float64} = Float64[]  # (gN/m2) initial fine root N for SASU
    frootn0_storage_patch             ::Vector{Float64} = Float64[]  # (gN/m2) initial fine root N storage for SASU
    frootn0_xfer_patch                ::Vector{Float64} = Float64[]  # (gN/m2) initial fine root N transfer for SASU
    livestemn0_patch                  ::Vector{Float64} = Float64[]  # (gN/m2) initial live stem N for SASU
    livestemn0_storage_patch          ::Vector{Float64} = Float64[]  # (gN/m2) initial live stem N storage for SASU
    livestemn0_xfer_patch             ::Vector{Float64} = Float64[]  # (gN/m2) initial live stem N transfer for SASU
    deadstemn0_patch                  ::Vector{Float64} = Float64[]  # (gN/m2) initial dead stem N for SASU
    deadstemn0_storage_patch          ::Vector{Float64} = Float64[]  # (gN/m2) initial dead stem N storage for SASU
    deadstemn0_xfer_patch             ::Vector{Float64} = Float64[]  # (gN/m2) initial dead stem N transfer for SASU
    livecrootn0_patch                 ::Vector{Float64} = Float64[]  # (gN/m2) initial live coarse root N for SASU
    livecrootn0_storage_patch         ::Vector{Float64} = Float64[]  # (gN/m2) initial live coarse root N storage for SASU
    livecrootn0_xfer_patch            ::Vector{Float64} = Float64[]  # (gN/m2) initial live coarse root N transfer for SASU
    deadcrootn0_patch                 ::Vector{Float64} = Float64[]  # (gN/m2) initial dead coarse root N for SASU
    deadcrootn0_storage_patch         ::Vector{Float64} = Float64[]  # (gN/m2) initial dead coarse root N storage for SASU
    deadcrootn0_xfer_patch            ::Vector{Float64} = Float64[]  # (gN/m2) initial dead coarse root N transfer for SASU
    retransn0_patch                   ::Vector{Float64} = Float64[]  # (gN/m2) initial retranslocated N for SASU
    repron0_patch                     ::Vector{Float64} = Float64[]  # (gN/m2) initial grain N for SASU
    repron0_storage_patch             ::Vector{Float64} = Float64[]  # (gN/m2) initial grain N storage for SASU
    repron0_xfer_patch                ::Vector{Float64} = Float64[]  # (gN/m2) initial grain N transfer for SASU

    # --- SASU save fields ---
    leafn_SASUsave_patch              ::Vector{Float64} = Float64[]
    leafn_storage_SASUsave_patch      ::Vector{Float64} = Float64[]
    leafn_xfer_SASUsave_patch         ::Vector{Float64} = Float64[]
    frootn_SASUsave_patch             ::Vector{Float64} = Float64[]
    frootn_storage_SASUsave_patch     ::Vector{Float64} = Float64[]
    frootn_xfer_SASUsave_patch        ::Vector{Float64} = Float64[]
    livestemn_SASUsave_patch          ::Vector{Float64} = Float64[]
    livestemn_storage_SASUsave_patch  ::Vector{Float64} = Float64[]
    livestemn_xfer_SASUsave_patch     ::Vector{Float64} = Float64[]
    deadstemn_SASUsave_patch          ::Vector{Float64} = Float64[]
    deadstemn_storage_SASUsave_patch  ::Vector{Float64} = Float64[]
    deadstemn_xfer_SASUsave_patch     ::Vector{Float64} = Float64[]
    livecrootn_SASUsave_patch         ::Vector{Float64} = Float64[]
    livecrootn_storage_SASUsave_patch ::Vector{Float64} = Float64[]
    livecrootn_xfer_SASUsave_patch    ::Vector{Float64} = Float64[]
    deadcrootn_SASUsave_patch         ::Vector{Float64} = Float64[]
    deadcrootn_storage_SASUsave_patch ::Vector{Float64} = Float64[]
    deadcrootn_xfer_SASUsave_patch    ::Vector{Float64} = Float64[]
    grainn_SASUsave_patch             ::Vector{Float64} = Float64[]
    grainn_storage_SASUsave_patch     ::Vector{Float64} = Float64[]

    # --- Matrix N allocation accumulation variables (annual, patch-level) ---
    matrix_nalloc_leaf_acc_patch        ::Vector{Float64} = Float64[]
    matrix_nalloc_leafst_acc_patch      ::Vector{Float64} = Float64[]
    matrix_nalloc_froot_acc_patch       ::Vector{Float64} = Float64[]
    matrix_nalloc_frootst_acc_patch     ::Vector{Float64} = Float64[]
    matrix_nalloc_livestem_acc_patch    ::Vector{Float64} = Float64[]
    matrix_nalloc_livestemst_acc_patch  ::Vector{Float64} = Float64[]
    matrix_nalloc_deadstem_acc_patch    ::Vector{Float64} = Float64[]
    matrix_nalloc_deadstemst_acc_patch  ::Vector{Float64} = Float64[]
    matrix_nalloc_livecroot_acc_patch   ::Vector{Float64} = Float64[]
    matrix_nalloc_livecrootst_acc_patch ::Vector{Float64} = Float64[]
    matrix_nalloc_deadcroot_acc_patch   ::Vector{Float64} = Float64[]
    matrix_nalloc_deadcrootst_acc_patch ::Vector{Float64} = Float64[]
    matrix_nalloc_grain_acc_patch       ::Vector{Float64} = Float64[]
    matrix_nalloc_grainst_acc_patch     ::Vector{Float64} = Float64[]

    # --- Matrix N transfer accumulation (annual, patch-level) ---
    matrix_ntransfer_leafst_to_leafxf_acc_patch           ::Vector{Float64} = Float64[]
    matrix_ntransfer_leafxf_to_leaf_acc_patch             ::Vector{Float64} = Float64[]
    matrix_ntransfer_frootst_to_frootxf_acc_patch         ::Vector{Float64} = Float64[]
    matrix_ntransfer_frootxf_to_froot_acc_patch           ::Vector{Float64} = Float64[]
    matrix_ntransfer_livestemst_to_livestemxf_acc_patch    ::Vector{Float64} = Float64[]
    matrix_ntransfer_livestemxf_to_livestem_acc_patch      ::Vector{Float64} = Float64[]
    matrix_ntransfer_deadstemst_to_deadstemxf_acc_patch    ::Vector{Float64} = Float64[]
    matrix_ntransfer_deadstemxf_to_deadstem_acc_patch      ::Vector{Float64} = Float64[]
    matrix_ntransfer_livecrootst_to_livecrootxf_acc_patch ::Vector{Float64} = Float64[]
    matrix_ntransfer_livecrootxf_to_livecroot_acc_patch   ::Vector{Float64} = Float64[]
    matrix_ntransfer_deadcrootst_to_deadcrootxf_acc_patch ::Vector{Float64} = Float64[]
    matrix_ntransfer_deadcrootxf_to_deadcroot_acc_patch   ::Vector{Float64} = Float64[]
    matrix_ntransfer_grainst_to_grainxf_acc_patch         ::Vector{Float64} = Float64[]
    matrix_ntransfer_grainxf_to_grain_acc_patch           ::Vector{Float64} = Float64[]
    matrix_ntransfer_livestem_to_deadstem_acc_patch        ::Vector{Float64} = Float64[]
    matrix_ntransfer_livecroot_to_deadcroot_acc_patch      ::Vector{Float64} = Float64[]

    # --- Matrix N retranslocation transfer accumulation (annual, patch-level) ---
    matrix_ntransfer_retransn_to_leaf_acc_patch           ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_leafst_acc_patch         ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_froot_acc_patch          ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_frootst_acc_patch        ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_livestem_acc_patch       ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_livestemst_acc_patch     ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_deadstem_acc_patch       ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_deadstemst_acc_patch     ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_livecroot_acc_patch      ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_livecrootst_acc_patch    ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_deadcroot_acc_patch      ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_deadcrootst_acc_patch    ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_grain_acc_patch          ::Vector{Float64} = Float64[]
    matrix_ntransfer_retransn_to_grainst_acc_patch        ::Vector{Float64} = Float64[]

    # --- Matrix N from-pool to retranslocation transfer accumulation ---
    matrix_ntransfer_leaf_to_retransn_acc_patch           ::Vector{Float64} = Float64[]
    matrix_ntransfer_froot_to_retransn_acc_patch          ::Vector{Float64} = Float64[]
    matrix_ntransfer_livestem_to_retransn_acc_patch       ::Vector{Float64} = Float64[]
    matrix_ntransfer_livecroot_to_retransn_acc_patch      ::Vector{Float64} = Float64[]

    # --- Matrix N turnover accumulation (annual, patch-level) ---
    matrix_nturnover_leaf_acc_patch             ::Vector{Float64} = Float64[]
    matrix_nturnover_leafst_acc_patch           ::Vector{Float64} = Float64[]
    matrix_nturnover_leafxf_acc_patch           ::Vector{Float64} = Float64[]
    matrix_nturnover_froot_acc_patch            ::Vector{Float64} = Float64[]
    matrix_nturnover_frootst_acc_patch          ::Vector{Float64} = Float64[]
    matrix_nturnover_frootxf_acc_patch          ::Vector{Float64} = Float64[]
    matrix_nturnover_livestem_acc_patch         ::Vector{Float64} = Float64[]
    matrix_nturnover_livestemst_acc_patch       ::Vector{Float64} = Float64[]
    matrix_nturnover_livestemxf_acc_patch       ::Vector{Float64} = Float64[]
    matrix_nturnover_deadstem_acc_patch         ::Vector{Float64} = Float64[]
    matrix_nturnover_deadstemst_acc_patch       ::Vector{Float64} = Float64[]
    matrix_nturnover_deadstemxf_acc_patch       ::Vector{Float64} = Float64[]
    matrix_nturnover_livecroot_acc_patch        ::Vector{Float64} = Float64[]
    matrix_nturnover_livecrootst_acc_patch      ::Vector{Float64} = Float64[]
    matrix_nturnover_livecrootxf_acc_patch      ::Vector{Float64} = Float64[]
    matrix_nturnover_deadcroot_acc_patch        ::Vector{Float64} = Float64[]
    matrix_nturnover_deadcrootst_acc_patch      ::Vector{Float64} = Float64[]
    matrix_nturnover_deadcrootxf_acc_patch      ::Vector{Float64} = Float64[]
    matrix_nturnover_grain_acc_patch            ::Vector{Float64} = Float64[]
    matrix_nturnover_grainst_acc_patch          ::Vector{Float64} = Float64[]
    matrix_nturnover_grainxf_acc_patch          ::Vector{Float64} = Float64[]
    matrix_nturnover_retransn_acc_patch         ::Vector{Float64} = Float64[]

    # --- Summary (diagnostic) state variables ---
    dispvegn_patch                    ::Vector{Float64} = Float64[]  # (gN/m2) displayed veg nitrogen, excluding storage
    storvegn_patch                    ::Vector{Float64} = Float64[]  # (gN/m2) stored vegetation nitrogen
    totvegn_patch                     ::Vector{Float64} = Float64[]  # (gN/m2) total vegetation nitrogen
    totn_patch                        ::Vector{Float64} = Float64[]  # (gN/m2) total patch-level nitrogen

    # --- Column-level fields ---
    totvegn_col                       ::Vector{Float64} = Float64[]  # (gN/m2) total vegetation nitrogen (p2c)
    totn_p2c_col                      ::Vector{Float64} = Float64[]  # (gN/m2) totn_patch averaged to column

    # --- Gridcell-level ---
    seedn_grc                         ::Vector{Float64} = Float64[]  # (gN/m2) pool for seeding new PFTs via dynamic landcover
end
