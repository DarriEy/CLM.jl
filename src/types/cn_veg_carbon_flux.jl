...
"""
Base.@kwdef mutable struct CNVegCarbonFluxData
    dribble_crophrv_xsmrpool_2atm::Bool = false

    # --- Gap mortality fluxes (gC/m2/s) patch-level ---
    m_leafc_to_litter_patch                   ::Vector{Float64} = Float64[]
    m_leafc_storage_to_litter_patch           ::Vector{Float64} = Float64[]
    m_leafc_xfer_to_litter_patch              ::Vector{Float64} = Float64[]
    m_frootc_to_litter_patch                  ::Vector{Float64} = Float64[]
    m_frootc_storage_to_litter_patch          ::Vector{Float64} = Float64[]
    m_frootc_xfer_to_litter_patch             ::Vector{Float64} = Float64[]
    m_livestemc_to_litter_patch               ::Vector{Float64} = Float64[]
    m_livestemc_storage_to_litter_patch       ::Vector{Float64} = Float64[]
    m_livestemc_xfer_to_litter_patch          ::Vector{Float64} = Float64[]
    m_deadstemc_to_litter_patch               ::Vector{Float64} = Float64[]
    m_deadstemc_storage_to_litter_patch       ::Vector{Float64} = Float64[]
    m_deadstemc_xfer_to_litter_patch          ::Vector{Float64} = Float64[]
    m_livecrootc_to_litter_patch              ::Vector{Float64} = Float64[]
    m_livecrootc_storage_to_litter_patch      ::Vector{Float64} = Float64[]
    m_livecrootc_xfer_to_litter_patch         ::Vector{Float64} = Float64[]
    m_deadcrootc_to_litter_patch              ::Vector{Float64} = Float64[]
    m_deadcrootc_storage_to_litter_patch      ::Vector{Float64} = Float64[]
    m_deadcrootc_xfer_to_litter_patch         ::Vector{Float64} = Float64[]
    m_gresp_storage_to_litter_patch           ::Vector{Float64} = Float64[]
    m_gresp_xfer_to_litter_patch              ::Vector{Float64} = Float64[]

    # --- Harvest mortality fluxes (gC/m2/s) patch-level ---
    hrv_leafc_to_litter_patch                 ::Vector{Float64} = Float64[]
    hrv_leafc_storage_to_litter_patch         ::Vector{Float64} = Float64[]
    hrv_leafc_xfer_to_litter_patch            ::Vector{Float64} = Float64[]
    hrv_frootc_to_litter_patch                ::Vector{Float64} = Float64[]
    hrv_frootc_storage_to_litter_patch        ::Vector{Float64} = Float64[]
    hrv_frootc_xfer_to_litter_patch           ::Vector{Float64} = Float64[]
    hrv_livestemc_to_litter_patch             ::Vector{Float64} = Float64[]
    hrv_livestemc_storage_to_litter_patch     ::Vector{Float64} = Float64[]
    hrv_livestemc_xfer_to_litter_patch        ::Vector{Float64} = Float64[]
    hrv_deadstemc_storage_to_litter_patch     ::Vector{Float64} = Float64[]
    hrv_deadstemc_xfer_to_litter_patch        ::Vector{Float64} = Float64[]
    hrv_livecrootc_to_litter_patch            ::Vector{Float64} = Float64[]
    hrv_livecrootc_storage_to_litter_patch    ::Vector{Float64} = Float64[]
    hrv_livecrootc_xfer_to_litter_patch       ::Vector{Float64} = Float64[]
    hrv_deadcrootc_to_litter_patch            ::Vector{Float64} = Float64[]
    hrv_deadcrootc_storage_to_litter_patch    ::Vector{Float64} = Float64[]
    hrv_deadcrootc_xfer_to_litter_patch       ::Vector{Float64} = Float64[]
    hrv_gresp_storage_to_litter_patch         ::Vector{Float64} = Float64[]
    hrv_gresp_xfer_to_litter_patch            ::Vector{Float64} = Float64[]
    hrv_xsmrpool_to_atm_patch                 ::Vector{Float64} = Float64[]

    # --- Fire fluxes (gC/m2/s) patch-level ---
    m_leafc_to_fire_patch                     ::Vector{Float64} = Float64[]
    m_leafc_storage_to_fire_patch             ::Vector{Float64} = Float64[]
    m_leafc_xfer_to_fire_patch                ::Vector{Float64} = Float64[]
    m_livestemc_to_fire_patch                 ::Vector{Float64} = Float64[]
    m_livestemc_storage_to_fire_patch         ::Vector{Float64} = Float64[]
    m_livestemc_xfer_to_fire_patch            ::Vector{Float64} = Float64[]
    m_deadstemc_to_fire_patch                 ::Vector{Float64} = Float64[]
    m_deadstemc_storage_to_fire_patch         ::Vector{Float64} = Float64[]
    m_deadstemc_xfer_to_fire_patch            ::Vector{Float64} = Float64[]
    m_frootc_to_fire_patch                    ::Vector{Float64} = Float64[]
    m_frootc_storage_to_fire_patch            ::Vector{Float64} = Float64[]
    m_frootc_xfer_to_fire_patch               ::Vector{Float64} = Float64[]
    m_livecrootc_to_fire_patch                ::Vector{Float64} = Float64[]
    m_livecrootc_storage_to_fire_patch        ::Vector{Float64} = Float64[]
    m_livecrootc_xfer_to_fire_patch           ::Vector{Float64} = Float64[]
    m_deadcrootc_to_fire_patch                ::Vector{Float64} = Float64[]
    m_deadcrootc_storage_to_fire_patch        ::Vector{Float64} = Float64[]
    m_deadcrootc_xfer_to_fire_patch           ::Vector{Float64} = Float64[]
    m_gresp_storage_to_fire_patch             ::Vector{Float64} = Float64[]
    m_gresp_xfer_to_fire_patch                ::Vector{Float64} = Float64[]

    # --- Fire-to-litter fluxes (gC/m2/s) patch-level ---
    m_leafc_to_litter_fire_patch              ::Vector{Float64} = Float64[]
    m_leafc_storage_to_litter_fire_patch      ::Vector{Float64} = Float64[]
    m_leafc_xfer_to_litter_fire_patch         ::Vector{Float64} = Float64[]
    m_livestemc_to_litter_fire_patch          ::Vector{Float64} = Float64[]
    m_livestemc_storage_to_litter_fire_patch  ::Vector{Float64} = Float64[]
    m_livestemc_xfer_to_litter_fire_patch     ::Vector{Float64} = Float64[]
    m_livestemc_to_deadstemc_fire_patch       ::Vector{Float64} = Float64[]
    m_deadstemc_to_litter_fire_patch          ::Vector{Float64} = Float64[]
    m_deadstemc_storage_to_litter_fire_patch  ::Vector{Float64} = Float64[]
    m_deadstemc_xfer_to_litter_fire_patch     ::Vector{Float64} = Float64[]
    m_frootc_to_litter_fire_patch             ::Vector{Float64} = Float64[]
    m_frootc_storage_to_litter_fire_patch     ::Vector{Float64} = Float64[]
    m_frootc_xfer_to_litter_fire_patch        ::Vector{Float64} = Float64[]
    m_livecrootc_to_litter_fire_patch         ::Vector{Float64} = Float64[]
    m_livecrootc_storage_to_litter_fire_patch ::Vector{Float64} = Float64[]
    m_livecrootc_xfer_to_litter_fire_patch    ::Vector{Float64} = Float64[]
    m_livecrootc_to_deadcrootc_fire_patch     ::Vector{Float64} = Float64[]
    m_deadcrootc_to_litter_fire_patch         ::Vector{Float64} = Float64[]
    m_deadcrootc_storage_to_litter_fire_patch ::Vector{Float64} = Float64[]
    m_deadcrootc_xfer_to_litter_fire_patch    ::Vector{Float64} = Float64[]
    m_gresp_storage_to_litter_fire_patch      ::Vector{Float64} = Float64[]
    m_gresp_xfer_to_litter_fire_patch         ::Vector{Float64} = Float64[]

    # --- Phenology fluxes (gC/m2/s) ---
    reproductivec_xfer_to_reproductivec_patch ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    leafc_xfer_to_leafc_patch                 ::Vector{Float64} = Float64[]
    frootc_xfer_to_frootc_patch               ::Vector{Float64} = Float64[]
    livestemc_xfer_to_livestemc_patch         ::Vector{Float64} = Float64[]
    deadstemc_xfer_to_deadstemc_patch         ::Vector{Float64} = Float64[]
    livecrootc_xfer_to_livecrootc_patch       ::Vector{Float64} = Float64[]
    deadcrootc_xfer_to_deadcrootc_patch       ::Vector{Float64} = Float64[]

    # --- Leaf/froot litterfall and crop fluxes (gC/m2/s) ---
    leafc_to_litter_patch                     ::Vector{Float64} = Float64[]
    leafc_to_litter_fun_patch                 ::Vector{Float64} = Float64[]
    frootc_to_litter_patch                    ::Vector{Float64} = Float64[]
    livestemc_to_litter_patch                 ::Vector{Float64} = Float64[]
    repr_grainc_to_food_patch                 ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    repr_grainc_to_food_perharv_patch         ::Array{Float64,3} = Array{Float64}(undef, 0, 0, 0)
    repr_grainc_to_food_thisyr_patch          ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    repr_structurec_to_cropprod_patch         ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    repr_structurec_to_litter_patch           ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    leafc_to_biofuelc_patch                   ::Vector{Float64} = Float64[]
    livestemc_to_biofuelc_patch               ::Vector{Float64} = Float64[]
    leafc_to_removedresiduec_patch            ::Vector{Float64} = Float64[]
    livestemc_to_removedresiduec_patch        ::Vector{Float64} = Float64[]
    repr_grainc_to_seed_patch                 ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    repr_grainc_to_seed_perharv_patch         ::Array{Float64,3} = Array{Float64}(undef, 0, 0, 0)
    repr_grainc_to_seed_thisyr_patch          ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)

    # --- Maintenance respiration (gC/m2/s) ---
    cpool_to_resp_patch                       ::Vector{Float64} = Float64[]
    cpool_to_leafc_resp_patch                 ::Vector{Float64} = Float64[]
    cpool_to_leafc_storage_resp_patch         ::Vector{Float64} = Float64[]
    cpool_to_frootc_resp_patch                ::Vector{Float64} = Float64[]
    cpool_to_frootc_storage_resp_patch        ::Vector{Float64} = Float64[]
    cpool_to_livecrootc_resp_patch            ::Vector{Float64} = Float64[]
    cpool_to_livecrootc_storage_resp_patch    ::Vector{Float64} = Float64[]
    cpool_to_livestemc_resp_patch             ::Vector{Float64} = Float64[]
    cpool_to_livestemc_storage_resp_patch     ::Vector{Float64} = Float64[]
    leaf_mr_patch                             ::Vector{Float64} = Float64[]
    froot_mr_patch                            ::Vector{Float64} = Float64[]
    livestem_mr_patch                         ::Vector{Float64} = Float64[]
    livecroot_mr_patch                        ::Vector{Float64} = Float64[]
    reproductive_mr_patch                     ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    leaf_curmr_patch                          ::Vector{Float64} = Float64[]
    froot_curmr_patch                         ::Vector{Float64} = Float64[]
    livestem_curmr_patch                      ::Vector{Float64} = Float64[]
    livecroot_curmr_patch                     ::Vector{Float64} = Float64[]
    reproductive_curmr_patch                  ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    leaf_xsmr_patch                           ::Vector{Float64} = Float64[]
    froot_xsmr_patch                          ::Vector{Float64} = Float64[]
    livestem_xsmr_patch                       ::Vector{Float64} = Float64[]
    livecroot_xsmr_patch                      ::Vector{Float64} = Float64[]
    reproductive_xsmr_patch                   ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)

    # --- Photosynthesis (gC/m2/s) ---
    psnsun_to_cpool_patch                     ::Vector{Float64} = Float64[]
    psnshade_to_cpool_patch                   ::Vector{Float64} = Float64[]

    # --- Allocation from cpool (gC/m2/s) ---
    cpool_to_xsmrpool_patch                   ::Vector{Float64} = Float64[]
    cpool_to_reproductivec_patch              ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    cpool_to_reproductivec_storage_patch      ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    cpool_to_leafc_patch                      ::Vector{Float64} = Float64[]
    cpool_to_leafc_storage_patch              ::Vector{Float64} = Float64[]
    cpool_to_frootc_patch                     ::Vector{Float64} = Float64[]
    cpool_to_frootc_storage_patch             ::Vector{Float64} = Float64[]
    cpool_to_livestemc_patch                  ::Vector{Float64} = Float64[]
    cpool_to_livestemc_storage_patch          ::Vector{Float64} = Float64[]
    cpool_to_deadstemc_patch                  ::Vector{Float64} = Float64[]
    cpool_to_deadstemc_storage_patch          ::Vector{Float64} = Float64[]
    cpool_to_livecrootc_patch                 ::Vector{Float64} = Float64[]
    cpool_to_livecrootc_storage_patch         ::Vector{Float64} = Float64[]
    cpool_to_deadcrootc_patch                 ::Vector{Float64} = Float64[]
    cpool_to_deadcrootc_storage_patch         ::Vector{Float64} = Float64[]
    cpool_to_gresp_storage_patch              ::Vector{Float64} = Float64[]

    # --- Growth respiration (gC/m2/s) ---
    xsmrpool_to_atm_patch                     ::Vector{Float64} = Float64[]
    xsmrpool_to_atm_col                       ::Vector{Float64} = Float64[]
    xsmrpool_to_atm_grc                       ::Vector{Float64} = Float64[]
    cpool_leaf_gr_patch                       ::Vector{Float64} = Float64[]
    cpool_leaf_storage_gr_patch               ::Vector{Float64} = Float64[]
    transfer_leaf_gr_patch                    ::Vector{Float64} = Float64[]
    cpool_froot_gr_patch                      ::Vector{Float64} = Float64[]
    cpool_froot_storage_gr_patch              ::Vector{Float64} = Float64[]
    transfer_froot_gr_patch                   ::Vector{Float64} = Float64[]
    cpool_livestem_gr_patch                   ::Vector{Float64} = Float64[]
    cpool_livestem_storage_gr_patch           ::Vector{Float64} = Float64[]
    transfer_livestem_gr_patch                ::Vector{Float64} = Float64[]
    cpool_deadstem_gr_patch                   ::Vector{Float64} = Float64[]
    cpool_deadstem_storage_gr_patch           ::Vector{Float64} = Float64[]
    transfer_deadstem_gr_patch                ::Vector{Float64} = Float64[]
    cpool_livecroot_gr_patch                  ::Vector{Float64} = Float64[]
    cpool_livecroot_storage_gr_patch          ::Vector{Float64} = Float64[]
    transfer_livecroot_gr_patch               ::Vector{Float64} = Float64[]
    cpool_deadcroot_gr_patch                  ::Vector{Float64} = Float64[]
    cpool_deadcroot_storage_gr_patch          ::Vector{Float64} = Float64[]
    transfer_deadcroot_gr_patch               ::Vector{Float64} = Float64[]
    cpool_reproductive_gr_patch               ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    cpool_reproductive_storage_gr_patch       ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    transfer_reproductive_gr_patch            ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)

    # --- Storage to transfer turnover (gC/m2/s) ---
    reproductivec_storage_to_xfer_patch       ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    leafc_storage_to_xfer_patch               ::Vector{Float64} = Float64[]
    frootc_storage_to_xfer_patch              ::Vector{Float64} = Float64[]
    livestemc_storage_to_xfer_patch           ::Vector{Float64} = Float64[]
    deadstemc_storage_to_xfer_patch           ::Vector{Float64} = Float64[]
    livecrootc_storage_to_xfer_patch          ::Vector{Float64} = Float64[]
    deadcrootc_storage_to_xfer_patch          ::Vector{Float64} = Float64[]
    gresp_storage_to_xfer_patch               ::Vector{Float64} = Float64[]

    # --- Livewood to deadwood turnover (gC/m2/s) ---
    livestemc_to_deadstemc_patch              ::Vector{Float64} = Float64[]
    livecrootc_to_deadcrootc_patch            ::Vector{Float64} = Float64[]

    # --- Column-level decomposition fluxes (gC/m3/s) ---
    phenology_c_to_litr_c_col                 ::Array{Float64,3} = Array{Float64}(undef, 0, 0, 0)
    gap_mortality_c_to_litr_c_col             ::Array{Float64,3} = Array{Float64}(undef, 0, 0, 0)
    gap_mortality_c_to_cwdc_col               ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    fire_mortality_c_to_cwdc_col              ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    harvest_c_to_litr_c_col                   ::Array{Float64,3} = Array{Float64}(undef, 0, 0, 0)
    harvest_c_to_cwdc_col                     ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    crop_harvestc_to_cropprodc_patch          ::Vector{Float64} = Float64[]
    crop_harvestc_to_cropprodc_col            ::Vector{Float64} = Float64[]
    m_decomp_cpools_to_fire_vr_col            ::Array{Float64,3} = Array{Float64}(undef, 0, 0, 0)
    m_decomp_cpools_to_fire_col               ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    m_c_to_litr_fire_col                      ::Array{Float64,3} = Array{Float64}(undef, 0, 0, 0)

    # --- Dynamic landcover fluxes ---
    dwt_seedc_to_leaf_patch                   ::Vector{Float64} = Float64[]
    dwt_seedc_to_leaf_grc                     ::Vector{Float64} = Float64[]
    dwt_seedc_to_deadstem_patch               ::Vector{Float64} = Float64[]
    dwt_seedc_to_deadstem_grc                 ::Vector{Float64} = Float64[]
    dwt_conv_cflux_patch                      ::Vector{Float64} = Float64[]
    dwt_conv_cflux_grc                        ::Vector{Float64} = Float64[]
    dwt_conv_cflux_dribbled_grc               ::Vector{Float64} = Float64[]
    dwt_wood_productc_gain_patch              ::Vector{Float64} = Float64[]
    dwt_crop_productc_gain_patch              ::Vector{Float64} = Float64[]
    dwt_slash_cflux_patch                     ::Vector{Float64} = Float64[]
    dwt_slash_cflux_grc                       ::Vector{Float64} = Float64[]
    dwt_frootc_to_litr_c_col                  ::Array{Float64,3} = Array{Float64}(undef, 0, 0, 0)
    dwt_livecrootc_to_cwdc_col                ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    dwt_deadcrootc_to_cwdc_col                ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)

    # --- Gross unrepresented landcover fluxes ---
    gru_leafc_to_litter_patch                 ::Vector{Float64} = Float64[]
    gru_leafc_storage_to_atm_patch            ::Vector{Float64} = Float64[]
    gru_leafc_xfer_to_atm_patch               ::Vector{Float64} = Float64[]
    gru_frootc_to_litter_patch                ::Vector{Float64} = Float64[]
    gru_frootc_storage_to_atm_patch           ::Vector{Float64} = Float64[]
    gru_frootc_xfer_to_atm_patch              ::Vector{Float64} = Float64[]
    gru_livestemc_to_atm_patch                ::Vector{Float64} = Float64[]
    gru_livestemc_storage_to_atm_patch        ::Vector{Float64} = Float64[]
    gru_livestemc_xfer_to_atm_patch           ::Vector{Float64} = Float64[]
    gru_deadstemc_to_atm_patch                ::Vector{Float64} = Float64[]
    gru_deadstemc_storage_to_atm_patch        ::Vector{Float64} = Float64[]
    gru_deadstemc_xfer_to_atm_patch           ::Vector{Float64} = Float64[]
    gru_livecrootc_to_litter_patch            ::Vector{Float64} = Float64[]
    gru_livecrootc_storage_to_atm_patch       ::Vector{Float64} = Float64[]
    gru_livecrootc_xfer_to_atm_patch          ::Vector{Float64} = Float64[]
    gru_deadcrootc_to_litter_patch            ::Vector{Float64} = Float64[]
    gru_deadcrootc_storage_to_atm_patch       ::Vector{Float64} = Float64[]
    gru_deadcrootc_xfer_to_atm_patch          ::Vector{Float64} = Float64[]
    gru_gresp_storage_to_atm_patch            ::Vector{Float64} = Float64[]
    gru_gresp_xfer_to_atm_patch               ::Vector{Float64} = Float64[]
    gru_xsmrpool_to_atm_patch                 ::Vector{Float64} = Float64[]
    gru_conv_cflux_patch                      ::Vector{Float64} = Float64[]
    gru_conv_cflux_col                        ::Vector{Float64} = Float64[]
    gru_conv_cflux_grc                        ::Vector{Float64} = Float64[]
    gru_conv_cflux_dribbled_grc               ::Vector{Float64} = Float64[]
    gru_wood_productc_gain_patch              ::Vector{Float64} = Float64[]
    gru_wood_productc_gain_col                ::Vector{Float64} = Float64[]
    gru_slash_cflux_patch                     ::Vector{Float64} = Float64[]
    gru_c_to_litr_c_col                       ::Array{Float64,3} = Array{Float64}(undef, 0, 0, 0)
    gru_c_to_cwdc_col                         ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)

    # --- Crop fluxes ---
    crop_seedc_to_leaf_patch                  ::Vector{Float64} = Float64[]

    # --- Summary/diagnostic fluxes (gC/m2/s) ---
    gpp_before_downreg_patch                  ::Vector{Float64} = Float64[]
    current_gr_patch                          ::Vector{Float64} = Float64[]
    transfer_gr_patch                         ::Vector{Float64} = Float64[]
    storage_gr_patch                          ::Vector{Float64} = Float64[]
    plant_calloc_patch                        ::Vector{Float64} = Float64[]
    excess_cflux_patch                        ::Vector{Float64} = Float64[]
    prev_leafc_to_litter_patch                ::Vector{Float64} = Float64[]
    prev_frootc_to_litter_patch               ::Vector{Float64} = Float64[]
    availc_patch                              ::Vector{Float64} = Float64[]
    xsmrpool_recover_patch                    ::Vector{Float64} = Float64[]
    xsmrpool_c13ratio_patch                   ::Vector{Float64} = Float64[]
    cwdc_loss_col                             ::Vector{Float64} = Float64[]
    litterc_loss_col                          ::Vector{Float64} = Float64[]
    frootc_alloc_patch                        ::Vector{Float64} = Float64[]
    frootc_loss_patch                         ::Vector{Float64} = Float64[]
    leafc_alloc_patch                         ::Vector{Float64} = Float64[]
    leafc_loss_patch                          ::Vector{Float64} = Float64[]
    woodc_alloc_patch                         ::Vector{Float64} = Float64[]
    woodc_loss_patch                          ::Vector{Float64} = Float64[]
    gpp_patch                                 ::Vector{Float64} = Float64[]
    gpp_col                                   ::Vector{Float64} = Float64[]
    rr_patch                                  ::Vector{Float64} = Float64[]
    rr_col                                    ::Vector{Float64} = Float64[]
    mr_patch                                  ::Vector{Float64} = Float64[]
    gr_patch                                  ::Vector{Float64} = Float64[]
    ar_patch                                  ::Vector{Float64} = Float64[]
    ar_col                                    ::Vector{Float64} = Float64[]
    npp_patch                                 ::Vector{Float64} = Float64[]
    npp_col                                   ::Vector{Float64} = Float64[]
    agnpp_patch                               ::Vector{Float64} = Float64[]
    bgnpp_patch                               ::Vector{Float64} = Float64[]
    litfall_patch                             ::Vector{Float64} = Float64[]
    wood_harvestc_patch                       ::Vector{Float64} = Float64[]
    wood_harvestc_col                         ::Vector{Float64} = Float64[]
    slash_harvestc_patch                      ::Vector{Float64} = Float64[]
    cinputs_patch                             ::Vector{Float64} = Float64[]
    coutputs_patch                            ::Vector{Float64} = Float64[]
    sr_col                                    ::Vector{Float64} = Float64[]
    er_col                                    ::Vector{Float64} = Float64[]
    litfire_col                               ::Vector{Float64} = Float64[]
    somfire_col                               ::Vector{Float64} = Float64[]
    totfire_col                               ::Vector{Float64} = Float64[]
    hrv_xsmrpool_to_atm_col                  ::Vector{Float64} = Float64[]

    # --- Fire summary ---
    fire_closs_patch                          ::Vector{Float64} = Float64[]
    fire_closs_p2c_col                        ::Vector{Float64} = Float64[]
    fire_closs_col                            ::Vector{Float64} = Float64[]

    # --- Annual sums ---
    tempsum_litfall_patch                     ::Vector{Float64} = Float64[]
    annsum_litfall_patch                      ::Vector{Float64} = Float64[]
    tempsum_npp_patch                         ::Vector{Float64} = Float64[]
    annsum_npp_patch                          ::Vector{Float64} = Float64[]
    annsum_npp_col                            ::Vector{Float64} = Float64[]
    lag_npp_col                               ::Vector{Float64} = Float64[]

    # --- Summary C fluxes ---
    nep_col                                   ::Vector{Float64} = Float64[]
    nbp_grc                                   ::Vector{Float64} = Float64[]
    nee_grc                                   ::Vector{Float64} = Float64[]
    landuseflux_grc                           ::Vector{Float64} = Float64[]

    # --- FUN fluxes (gC/m2/s) patch-level ---
    npp_Nactive_patch                         ::Vector{Float64} = Float64[]
    npp_burnedoff_patch                       ::Vector{Float64} = Float64[]
    npp_Nnonmyc_patch                         ::Vector{Float64} = Float64[]
    npp_Nam_patch                             ::Vector{Float64} = Float64[]
    npp_Necm_patch                            ::Vector{Float64} = Float64[]
    npp_Nactive_no3_patch                     ::Vector{Float64} = Float64[]
    npp_Nactive_nh4_patch                     ::Vector{Float64} = Float64[]
    npp_Nnonmyc_no3_patch                     ::Vector{Float64} = Float64[]
    npp_Nnonmyc_nh4_patch                     ::Vector{Float64} = Float64[]
    npp_Nam_no3_patch                         ::Vector{Float64} = Float64[]
    npp_Nam_nh4_patch                         ::Vector{Float64} = Float64[]
    npp_Necm_no3_patch                        ::Vector{Float64} = Float64[]
    npp_Necm_nh4_patch                        ::Vector{Float64} = Float64[]
    npp_Nfix_patch                            ::Vector{Float64} = Float64[]
    npp_Nretrans_patch                        ::Vector{Float64} = Float64[]
    npp_Nuptake_patch                         ::Vector{Float64} = Float64[]
    npp_growth_patch                          ::Vector{Float64} = Float64[]
    leafc_change_patch                        ::Vector{Float64} = Float64[]
    soilc_change_patch                        ::Vector{Float64} = Float64[]

    # --- Matrix CN arrays ---
    matrix_Cinput_patch                       ::Vector{Float64} = Float64[]
    matrix_C13input_patch                     ::Vector{Float64} = Float64[]
    matrix_C14input_patch                     ::Vector{Float64} = Float64[]
    matrix_alloc_patch                        ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    matrix_phtransfer_patch                   ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    matrix_phturnover_patch                   ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    matrix_phtransfer_doner_patch             ::Vector{Int} = Int[]
    matrix_phtransfer_receiver_patch          ::Vector{Int} = Int[]
    matrix_gmtransfer_patch                   ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    matrix_gmturnover_patch                   ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    matrix_gmtransfer_doner_patch             ::Vector{Int} = Int[]
    matrix_gmtransfer_receiver_patch          ::Vector{Int} = Int[]
    matrix_fitransfer_patch                   ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
    matrix_fiturnover_patch                   ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)
...
function cnveg_carbon_flux_init_history!(cf::CNVegCarbonFluxData, args...)
    nothing
end

function cnveg_carbon_flux_restart!(cf::CNVegCarbonFluxData, args...)
    nothing
end
