...
"""
Base.@kwdef mutable struct CNVegCarbonStateData
    species::Int = 0  # carbon species: 1=C12, 2=C13, 3=C14

    # --- Patch-level 2D (patch x nrepr) ---
    reproductivec_patch               ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)  # (gC/m2) reproductive (grain) C
    reproductivec_storage_patch       ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)  # (gC/m2) reproductive C storage
    reproductivec_xfer_patch          ::Matrix{Float64} = Matrix{Float64}(undef, 0, 0)  # (gC/m2) reproductive C transfer

    # --- Patch-level 1D core pools ---
    leafc_patch                       ::Vector{Float64} = Float64[]  # (gC/m2) leaf C
    leafc_storage_patch               ::Vector{Float64} = Float64[]  # (gC/m2) leaf C storage
    leafc_xfer_patch                  ::Vector{Float64} = Float64[]  # (gC/m2) leaf C transfer
    leafc_storage_xfer_acc_patch      ::Vector{Float64} = Float64[]  # (gC/m2) accumulated leaf C transfer
    storage_cdemand_patch             ::Vector{Float64} = Float64[]  # (gC/m2) C use from storage pool
    frootc_patch                      ::Vector{Float64} = Float64[]  # (gC/m2) fine root C
    frootc_storage_patch              ::Vector{Float64} = Float64[]  # (gC/m2) fine root C storage
    frootc_xfer_patch                 ::Vector{Float64} = Float64[]  # (gC/m2) fine root C transfer
    livestemc_patch                   ::Vector{Float64} = Float64[]  # (gC/m2) live stem C
    livestemc_storage_patch           ::Vector{Float64} = Float64[]  # (gC/m2) live stem C storage
    livestemc_xfer_patch              ::Vector{Float64} = Float64[]  # (gC/m2) live stem C transfer
    deadstemc_patch                   ::Vector{Float64} = Float64[]  # (gC/m2) dead stem C
    deadstemc_storage_patch           ::Vector{Float64} = Float64[]  # (gC/m2) dead stem C storage
    deadstemc_xfer_patch              ::Vector{Float64} = Float64[]  # (gC/m2) dead stem C transfer
    livecrootc_patch                  ::Vector{Float64} = Float64[]  # (gC/m2) live coarse root C
    livecrootc_storage_patch          ::Vector{Float64} = Float64[]  # (gC/m2) live coarse root C storage
    livecrootc_xfer_patch             ::Vector{Float64} = Float64[]  # (gC/m2) live coarse root C transfer
    deadcrootc_patch                  ::Vector{Float64} = Float64[]  # (gC/m2) dead coarse root C
    deadcrootc_storage_patch          ::Vector{Float64} = Float64[]  # (gC/m2) dead coarse root C storage
    deadcrootc_xfer_patch             ::Vector{Float64} = Float64[]  # (gC/m2) dead coarse root C transfer
    gresp_storage_patch               ::Vector{Float64} = Float64[]  # (gC/m2) growth respiration storage
    gresp_xfer_patch                  ::Vector{Float64} = Float64[]  # (gC/m2) growth respiration transfer
    cpool_patch                       ::Vector{Float64} = Float64[]  # (gC/m2) temporary photosynthate C pool
    xsmrpool_patch                    ::Vector{Float64} = Float64[]  # (gC/m2) abstract C pool for excess MR demand
    xsmrpool_loss_patch               ::Vector{Float64} = Float64[]  # (gC/m2) abstract C pool excess MR demand loss
    ctrunc_patch                      ::Vector{Float64} = Float64[]  # (gC/m2) patch-level sink for C truncation
    woodc_patch                       ::Vector{Float64} = Float64[]  # (gC/m2) wood C
    leafcmax_patch                    ::Vector{Float64} = Float64[]  # (gC/m2) annual max leaf C
    cropseedc_deficit_patch           ::Vector{Float64} = Float64[]  # (gC/m2) pool for seeding new crop growth (negative)

    # --- Matrix CN capacity fields (patch-level) ---
    matrix_cap_leafc_patch              ::Vector{Float64} = Float64[]  # (gC/m2) capacity of leaf C
    matrix_cap_leafc_storage_patch      ::Vector{Float64} = Float64[]  # (gC/m2) capacity of leaf C storage
    matrix_cap_leafc_xfer_patch         ::Vector{Float64} = Float64[]  # (gC/m2) capacity of leaf C transfer
    matrix_cap_frootc_patch             ::Vector{Float64} = Float64[]  # (gC/m2) capacity of fine root C
    matrix_cap_frootc_storage_patch     ::Vector{Float64} = Float64[]  # (gC/m2) capacity of fine root C storage
    matrix_cap_frootc_xfer_patch        ::Vector{Float64} = Float64[]  # (gC/m2) capacity of fine root C transfer
    matrix_cap_livestemc_patch          ::Vector{Float64} = Float64[]  # (gC/m2) capacity of live stem C
    matrix_cap_livestemc_storage_patch  ::Vector{Float64} = Float64[]  # (gC/m2) capacity of live stem C storage
    matrix_cap_livestemc_xfer_patch     ::Vector{Float64} = Float64[]  # (gC/m2) capacity of live stem C transfer
    matrix_cap_deadstemc_patch          ::Vector{Float64} = Float64[]  # (gC/m2) capacity of dead stem C
    matrix_cap_deadstemc_storage_patch  ::Vector{Float64} = Float64[]  # (gC/m2) capacity of dead stem C storage
    matrix_cap_deadstemc_xfer_patch     ::Vector{Float64} = Float64[]  # (gC/m2) capacity of dead stem C transfer
    matrix_cap_livecrootc_patch         ::Vector{Float64} = Float64[]  # (gC/m2) capacity of live coarse root C
    matrix_cap_livecrootc_storage_patch ::Vector{Float64} = Float64[]  # (gC/m2) capacity of live coarse root C storage
    matrix_cap_livecrootc_xfer_patch    ::Vector{Float64} = Float64[]  # (gC/m2) capacity of live coarse root C transfer
    matrix_cap_deadcrootc_patch         ::Vector{Float64} = Float64[]  # (gC/m2) capacity of dead coarse root C
    matrix_cap_deadcrootc_storage_patch ::Vector{Float64} = Float64[]  # (gC/m2) capacity of dead coarse root C storage
    matrix_cap_deadcrootc_xfer_patch    ::Vector{Float64} = Float64[]  # (gC/m2) capacity of dead coarse root C transfer
    matrix_cap_reproc_patch             ::Vector{Float64} = Float64[]  # (gC/m2) capacity of grain C
    matrix_cap_reproc_storage_patch     ::Vector{Float64} = Float64[]  # (gC/m2) capacity of grain storage C
    matrix_cap_reproc_xfer_patch        ::Vector{Float64} = Float64[]  # (gC/m2) capacity of grain transfer C

    # --- Initial pool size of year for matrix (SASU) ---
    leafc0_patch                      ::Vector{Float64} = Float64[]  # (gC/m2) initial leaf C for SASU
    leafc0_storage_patch              ::Vector{Float64} = Float64[]  # (gC/m2) initial leaf C storage for SASU
    leafc0_xfer_patch                 ::Vector{Float64} = Float64[]  # (gC/m2) initial leaf C transfer for SASU
    frootc0_patch                     ::Vector{Float64} = Float64[]  # (gC/m2) initial fine root C for SASU
    frootc0_storage_patch             ::Vector{Float64} = Float64[]  # (gC/m2) initial fine root C storage for SASU
    frootc0_xfer_patch                ::Vector{Float64} = Float64[]  # (gC/m2) initial fine root C transfer for SASU
    livestemc0_patch                  ::Vector{Float64} = Float64[]  # (gC/m2) initial live stem C for SASU
    livestemc0_storage_patch          ::Vector{Float64} = Float64[]  # (gC/m2) initial live stem C storage for SASU
    livestemc0_xfer_patch             ::Vector{Float64} = Float64[]  # (gC/m2) initial live stem C transfer for SASU
    deadstemc0_patch                  ::Vector{Float64} = Float64[]  # (gC/m2) initial dead stem C for SASU
    deadstemc0_storage_patch          ::Vector{Float64} = Float64[]  # (gC/m2) initial dead stem C storage for SASU
    deadstemc0_xfer_patch             ::Vector{Float64} = Float64[]  # (gC/m2) initial dead stem C transfer for SASU
    livecrootc0_patch                 ::Vector{Float64} = Float64[]  # (gC/m2) initial live coarse root C for SASU
    livecrootc0_storage_patch         ::Vector{Float64} = Float64[]  # (gC/m2) initial live coarse root C storage for SASU
    livecrootc0_xfer_patch            ::Vector{Float64} = Float64[]  # (gC/m2) initial live coarse root C transfer for SASU
    deadcrootc0_patch                 ::Vector{Float64} = Float64[]  # (gC/m2) initial dead coarse root C for SASU
    deadcrootc0_storage_patch         ::Vector{Float64} = Float64[]  # (gC/m2) initial dead coarse root C storage for SASU
    deadcrootc0_xfer_patch            ::Vector{Float64} = Float64[]  # (gC/m2) initial dead coarse root C transfer for SASU
    reproc0_patch                     ::Vector{Float64} = Float64[]  # (gC/m2) initial grain C for SASU
    reproc0_storage_patch             ::Vector{Float64} = Float64[]  # (gC/m2) initial grain C storage for SASU
    reproc0_xfer_patch                ::Vector{Float64} = Float64[]  # (gC/m2) initial grain C transfer for SASU

    # --- SASU save fields ---
    leafc_SASUsave_patch              ::Vector{Float64} = Float64[]
    leafc_storage_SASUsave_patch      ::Vector{Float64} = Float64[]
    leafc_xfer_SASUsave_patch         ::Vector{Float64} = Float64[]
    frootc_SASUsave_patch             ::Vector{Float64} = Float64[]
    frootc_storage_SASUsave_patch     ::Vector{Float64} = Float64[]
    frootc_xfer_SASUsave_patch        ::Vector{Float64} = Float64[]
    livestemc_SASUsave_patch          ::Vector{Float64} = Float64[]
    livestemc_storage_SASUsave_patch  ::Vector{Float64} = Float64[]
    livestemc_xfer_SASUsave_patch     ::Vector{Float64} = Float64[]
    deadstemc_SASUsave_patch          ::Vector{Float64} = Float64[]
    deadstemc_storage_SASUsave_patch  ::Vector{Float64} = Float64[]
    deadstemc_xfer_SASUsave_patch     ::Vector{Float64} = Float64[]
    livecrootc_SASUsave_patch         ::Vector{Float64} = Float64[]
    livecrootc_storage_SASUsave_patch ::Vector{Float64} = Float64[]
    livecrootc_xfer_SASUsave_patch    ::Vector{Float64} = Float64[]
    deadcrootc_SASUsave_patch         ::Vector{Float64} = Float64[]
    deadcrootc_storage_SASUsave_patch ::Vector{Float64} = Float64[]
    deadcrootc_xfer_SASUsave_patch    ::Vector{Float64} = Float64[]
    grainc_SASUsave_patch             ::Vector{Float64} = Float64[]
    grainc_storage_SASUsave_patch     ::Vector{Float64} = Float64[]

    # --- Matrix accumulation variables (annual, patch-level) ---
    matrix_calloc_leaf_acc_patch        ::Vector{Float64} = Float64[]
    matrix_calloc_leafst_acc_patch      ::Vector{Float64} = Float64[]
    matrix_calloc_froot_acc_patch       ::Vector{Float64} = Float64[]
    matrix_calloc_frootst_acc_patch     ::Vector{Float64} = Float64[]
    matrix_calloc_livestem_acc_patch    ::Vector{Float64} = Float64[]
    matrix_calloc_livestemst_acc_patch  ::Vector{Float64} = Float64[]
    matrix_calloc_deadstem_acc_patch    ::Vector{Float64} = Float64[]
    matrix_calloc_deadstemst_acc_patch  ::Vector{Float64} = Float64[]
    matrix_calloc_livecroot_acc_patch   ::Vector{Float64} = Float64[]
    matrix_calloc_livecrootst_acc_patch ::Vector{Float64} = Float64[]
    matrix_calloc_deadcroot_acc_patch   ::Vector{Float64} = Float64[]
    matrix_calloc_deadcrootst_acc_patch ::Vector{Float64} = Float64[]
    matrix_calloc_grain_acc_patch       ::Vector{Float64} = Float64[]
    matrix_calloc_grainst_acc_patch     ::Vector{Float64} = Float64[]

    # --- Matrix C transfer accumulation (annual, patch-level) ---
    matrix_ctransfer_leafst_to_leafxf_acc_patch           ::Vector{Float64} = Float64[]
    matrix_ctransfer_leafxf_to_leaf_acc_patch             ::Vector{Float64} = Float64[]
    matrix_ctransfer_frootst_to_frootxf_acc_patch         ::Vector{Float64} = Float64[]
    matrix_ctransfer_frootxf_to_froot_acc_patch           ::Vector{Float64} = Float64[]
    matrix_ctransfer_livestemst_to_livestemxf_acc_patch    ::Vector{Float64} = Float64[]
    matrix_ctransfer_livestemxf_to_livestem_acc_patch      ::Vector{Float64} = Float64[]
    matrix_ctransfer_deadstemst_to_deadstemxf_acc_patch    ::Vector{Float64} = Float64[]
    matrix_ctransfer_deadstemxf_to_deadstem_acc_patch      ::Vector{Float64} = Float64[]
    matrix_ctransfer_livecrootst_to_livecrootxf_acc_patch ::Vector{Float64} = Float64[]
    matrix_ctransfer_livecrootxf_to_livecroot_acc_patch   ::Vector{Float64} = Float64[]
    matrix_ctransfer_deadcrootst_to_deadcrootxf_acc_patch ::Vector{Float64} = Float64[]
    matrix_ctransfer_deadcrootxf_to_deadcroot_acc_patch   ::Vector{Float64} = Float64[]
    matrix_ctransfer_grainst_to_grainxf_acc_patch         ::Vector{Float64} = Float64[]
    matrix_ctransfer_grainxf_to_grain_acc_patch           ::Vector{Float64} = Float64[]
    matrix_ctransfer_livestem_to_deadstem_acc_patch        ::Vector{Float64} = Float64[]
    matrix_ctransfer_livecroot_to_deadcroot_acc_patch      ::Vector{Float64} = Float64[]

    # --- Matrix C turnover accumulation (annual, patch-level) ---
    matrix_cturnover_leaf_acc_patch             ::Vector{Float64} = Float64[]
    matrix_cturnover_leafst_acc_patch           ::Vector{Float64} = Float64[]
    matrix_cturnover_leafxf_acc_patch           ::Vector{Float64} = Float64[]
    matrix_cturnover_froot_acc_patch            ::Vector{Float64} = Float64[]
    matrix_cturnover_frootst_acc_patch          ::Vector{Float64} = Float64[]
    matrix_cturnover_frootxf_acc_patch          ::Vector{Float64} = Float64[]
    matrix_cturnover_livestem_acc_patch         ::Vector{Float64} = Float64[]
    matrix_cturnover_livestemst_acc_patch       ::Vector{Float64} = Float64[]
    matrix_cturnover_livestemxf_acc_patch       ::Vector{Float64} = Float64[]
    matrix_cturnover_deadstem_acc_patch         ::Vector{Float64} = Float64[]
    matrix_cturnover_deadstemst_acc_patch       ::Vector{Float64} = Float64[]
    matrix_cturnover_deadstemxf_acc_patch       ::Vector{Float64} = Float64[]
    matrix_cturnover_livecroot_acc_patch        ::Vector{Float64} = Float64[]
    matrix_cturnover_livecrootst_acc_patch      ::Vector{Float64} = Float64[]
    matrix_cturnover_livecrootxf_acc_patch      ::Vector{Float64} = Float64[]
    matrix_cturnover_deadcroot_acc_patch        ::Vector{Float64} = Float64[]
    matrix_cturnover_deadcrootst_acc_patch      ::Vector{Float64} = Float64[]
    matrix_cturnover_deadcrootxf_acc_patch      ::Vector{Float64} = Float64[]
    matrix_cturnover_grain_acc_patch            ::Vector{Float64} = Float64[]
    matrix_cturnover_grainst_acc_patch          ::Vector{Float64} = Float64[]
    matrix_cturnover_grainxf_acc_patch          ::Vector{Float64} = Float64[]

    # --- Column-level fields ---
    rootc_col                         ::Vector{Float64} = Float64[]  # (gC/m2) root carbon at column level (fire)
    leafc_col                         ::Vector{Float64} = Float64[]  # (gC/m2) column-level leafc (fire)
    deadstemc_col                     ::Vector{Float64} = Float64[]  # (gC/m2) column-level deadstemc (fire)
    fuelc_col                         ::Vector{Float64} = Float64[]  # fuel load outside cropland
    fuelc_crop_col                    ::Vector{Float64} = Float64[]  # fuel load for cropland

    # --- Gridcell-level ---
    seedc_grc                         ::Vector{Float64} = Float64[]  # (gC/m2) pool for seeding new PFTs via dynamic landcover

    # --- Summary (diagnostic) state variables ---
    dispvegc_patch                    ::Vector{Float64} = Float64[]  # (gC/m2) displayed veg carbon, excluding storage and cpool
    storvegc_patch                    ::Vector{Float64} = Float64[]  # (gC/m2) stored vegetation carbon, excluding cpool
    totc_patch                        ::Vector{Float64} = Float64[]  # (gC/m2) total patch-level carbon, including cpool
    totvegc_patch                     ::Vector{Float64} = Float64[]  # (gC/m2) total vegetation carbon, excluding cpool
    totvegc_col                       ::Vector{Float64} = Float64[]  # (gC/m2) total vegetation carbon at column level
    totc_p2c_col                      ::Vector{Float64} = Float64[]  # (gC/m2) totc_patch averaged to column
