...
"""
function column_init!(col::ColumnData{FT}, ncols::Int) where {FT}
    ...
    col.wtgcell     = fill(FT(NaN), ncols)
    col.landunit    = fill(ISPVAL, ncols)
    col.wtlunit     = fill(FT(NaN), ncols)
    ...
    col.dz          = fill(FT(NaN), ncols, nlev_dz)
    col.z           = fill(FT(NaN), ncols, nlev_dz)
    col.zi          = fill(FT(NaN), ncols, nlev_zi)
    col.zii         = fill(FT(NaN), ncols)
    col.lakedepth   = fill(FT(SPVAL), ncols)
    col.dz_lake     = fill(FT(NaN), ncols, nlevlak)
    col.z_lake      = fill(FT(NaN), ncols, nlevlak)
    col.col_ndx     = fill(ISPVAL, ncols)
    col.colu        = fill(ISPVAL, ncols)
    col.cold        = fill(ISPVAL, ncols)
    col.hillslope_ndx = fill(ISPVAL, ncols)
    col.hill_elev   = fill(FT(SPVAL), ncols)
    col.hill_slope  = fill(FT(SPVAL), ncols)
    col.hill_area   = fill(FT(SPVAL), ncols)
    col.hill_width  = fill(FT(SPVAL), ncols)
    col.hill_distance = fill(FT(SPVAL), ncols)
    col.hill_aspect = fill(FT(SPVAL), ncols)
    col.nbedrock    = fill(ISPVAL, ncols)
    col.levgrnd_class = fill(ISPVAL, ncols, nlevmaxurbgrnd)
    col.micro_sigma = fill(FT(NaN), ncols)
    col.topo_slope  = fill(FT(NaN), ncols)
    col.topo_std    = fill(FT(NaN), ncols)
...
"""
function column_clean!(col::ColumnData{FT}) where {FT}
    col.gridcell    = Int[]
    col.wtgcell     = FT[]
    col.landunit    = Int[]
    col.wtlunit     = FT[]
    ...
    col.snl         = Int[]
    col.dz          = Matrix{FT}(undef, 0, 0)
    col.z           = Matrix{FT}(undef, 0, 0)
    col.zi          = Matrix{FT}(undef, 0, 0)
    col.zii         = FT[]
    col.lakedepth   = FT[]
    col.dz_lake     = Matrix{FT}(undef, 0, 0)
    col.z_lake      = Matrix{FT}(undef, 0, 0)
    col.micro_sigma = FT[]
    col.topo_slope  = FT[]
    col.topo_std    = FT[]
    col.nbedrock    = Int[]
...
"""
function column_update_itype!(col::ColumnData{FT}, c::Int, itype::Int) where {FT}
